...
RUN groupadd -g ${SHARED_GROUP_ID} ${SHARED_GROUP_NAME} \
 ...
 && apt-get install --no-install-recommends --no-install-suggests -y $_PACKAGES $_TMP_PACKAGES  \
 && wget -O- https://repository.mrw.sh/PublicKey | apt-key add - \
 && apt-add-repository https://repository.mrw.sh \
 && apt-get update \
 && locale-gen ${LANG} \
 && update-locale LANG=${LANG} \
 && apt-get -y autoremove --allow-remove-essential --purge $_TMP_PACKAGES $_REMOVE_PACKAGES \
 && /cleanup.sh
